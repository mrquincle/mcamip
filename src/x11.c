...
if(debug_flag)
	{
	fprintf(stderr, "x11: color depth: %u bits\n", display_bits);
...
XCreateImage(dpy, DefaultVisualOfScreen(DefaultScreenOfDisplay(dpy)),\
	...
	ZPixmap, 0, malloc(xsize * ysize * 4), // depth / 8  (24 / 8)? just 2 be on the save side
	xsize, ysize, 8, 0);
...
if(!grab_ximage)
	{
	fprintf(stderr, "grab_ximage == NULL shouldn't be!\n");
	}
...
if(debug_flag)
	{
	fprintf(stderr, "closewin(): arg none\n");
	}
...
extern int color_depth_mode;

/* set color_depth to display bit if in auto */
if(color_depth_mode == 0) color_depth = display_bits;

return color_depth;
}/* end function wingetdepth */
